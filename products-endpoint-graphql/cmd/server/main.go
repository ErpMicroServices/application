...
import (
	...
	"github.com/erpmicroservices/products-endpoint-graphql/graph"
	"github.com/erpmicroservices/products-endpoint-graphql/graph/generated"
	...
	"github.com/erpmicroservices/products-endpoint-graphql/pkg/directives"
	"github.com/erpmicroservices/products-endpoint-graphql/pkg/middleware"
)
...
func main() {
	...
	logger := log.With().Str("service", "products-graphql-api").Logger()

	logger.Info().
		Str("version", getVersion()).
		Str("environment", cfg.Server.Environment).
		Int("port", cfg.Server.Port).
		Msg("Starting Products GraphQL API")
	...
	// Setup AI services
	aiServices := setupAIServices(cfg.AI, logger)

	// Setup GraphQL server
	graphqlServer := setupGraphQLServer(cfg, db, aiServices, logger)
...
func setupLogging(cfg config.LoggingConfig) {
	...
	log.Logger = log.Logger.With().
		Timestamp().
		Str("service", "products-graphql-api").
		Logger()
}
...
func setupAIServices(cfg config.AIConfig, logger zerolog.Logger) *AIServices {
	services := &AIServices{}

	if cfg.Enabled {
		if cfg.CategorizationEnabled {
			services.Categorization = ai.NewCategorizationService(
				logger.With().Str("service", "ai-categorization").Logger(),
				cfg.ModelName,
				cfg.ModelAPIKey,
			)
		}
		...
		if cfg.RecommendationEnabled {
			services.Recommendation = ai.NewRecommendationEngine(
				logger.With().Str("service", "ai-recommendations").Logger(),
				cfg.ModelName,
			)
		}

		logger.Info().
			Bool("categorization", cfg.CategorizationEnabled).
			Bool("recommendations", cfg.RecommendationEnabled).
			Bool("image_analysis", cfg.ImageAnalysisEnabled).
			Bool("pricing_optimization", cfg.PricingOptimizationEnabled).
			Str("model_provider", cfg.ModelProvider).
			Str("model_name", cfg.ModelName).
			Msg("AI services initialized")
	} else {
		logger.Info().Msg("AI services disabled")
	}

	return services
}

func setupGraphQLServer(cfg *config.Config, db *sqlx.DB, aiServices *AIServices, logger zerolog.Logger) *handler.Server {
...
func setupHTTPServer(cfg *config.Config, graphqlServer *handler.Server, logger zerolog.Logger) *http.Server {
	...
	router.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{"status":"healthy","service":"products-graphql-api","timestamp":"%s"}`, 
	...
	router.Get("/ready", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		fmt.Fprintf(w, `{"status":"ready","service":"products-graphql-api","timestamp":"%s"}`, 
			time.Now().UTC().Format(time.RFC3339))
	})
	...
	router.Get("/metrics", func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		fmt.Fprintf(w, "# Products GraphQL API Metrics\n")
		fmt.Fprintf(w, "products_api_uptime{service=\"products-graphql-api\"} %d\n", 
