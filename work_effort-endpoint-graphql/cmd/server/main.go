...
import (
	...
	"github.com/erpmicroservices/work_effort-endpoint-graphql/graph"
	"github.com/erpmicroservices/work_effort-endpoint-graphql/graph/generated"
	"github.com/erpmicroservices/work_effort-endpoint-graphql/internal/config"
	"github.com/erpmicroservices/work_effort-endpoint-graphql/pkg/directives"
	"github.com/erpmicroservices/work_effort-endpoint-graphql/pkg/middleware"
)
...
func main() {
	...
	logger := log.With().Str("service", "work_effort-graphql-api").Logger()
...
func setupLogging(cfg config.LoggingConfig) {
	...
	log.Logger = log.Logger.With().
		Timestamp().
		Str("service", "work_effort-graphql-api").
		Logger()
}
...
func setupHTTPServer(cfg *config.Config, graphqlServer *handler.Server, logger zerolog.Logger) *http.Server {
	...
	router.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{"status":"healthy","service":"work_effort-graphql-api","timestamp":"%s"}`, 
	...
	router.Get("/ready", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		fmt.Fprintf(w, `{"status":"ready","service":"work_effort-graphql-api","timestamp":"%s"}`, 
			time.Now().UTC().Format(time.RFC3339))
	})
	...
	router.Get("/metrics", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, "hr_api_uptime{service=\"work_effort-graphql-api\"} %d\n", 
