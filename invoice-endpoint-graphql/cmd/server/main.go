...
import (
	...
	"github.com/erpmicroservices/invoice-endpoint-graphql/graph"
	"github.com/erpmicroservices/invoice-endpoint-graphql/graph/generated"
	"github.com/erpmicroservices/invoice-endpoint-graphql/internal/config"
	"github.com/erpmicroservices/invoice-endpoint-graphql/pkg/directives"
	"github.com/erpmicroservices/invoice-endpoint-graphql/pkg/middleware"
)
...
func main() {
	...
	logger := log.With().Str("service", "invoice-graphql-api").Logger()
...
func setupLogging(cfg config.LoggingConfig) {
	...
	log.Logger = log.Logger.With().
		Timestamp().
		Str("service", "invoice-graphql-api").
		Logger()
}
...
func setupHTTPServer(cfg *config.Config, graphqlServer *handler.Server, logger zerolog.Logger) *http.Server {
	...
	router.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{"status":"healthy","service":"invoice-graphql-api","timestamp":"%s"}`, 
	...
	router.Get("/ready", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		fmt.Fprintf(w, `{"status":"ready","service":"invoice-graphql-api","timestamp":"%s"}`, 
			time.Now().UTC().Format(time.RFC3339))
	})
	...
	router.Get("/metrics", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, "hr_api_uptime{service=\"invoice-graphql-api\"} %d\n", 
